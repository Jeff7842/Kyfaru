---
name: pdf2plot
description: Extract numerical data from a vector PDF figure (a line/scatter chart) into a CSV, purely from PDF geometry — no image inspection or manual clicking. Use when the user wants to digitize data points from a PDF chart, pull the underlying dataset out of a plot in a PDF, or convert a PDF figure to CSV for analysis.
license: MIT License. Copyright (c) 2024 Adam Kucharski.
source: https://github.com/adamkucharski/pdf2plot
---

# pdf2plot skill

Extract data from a vector PDF figure end-to-end — no human clicking required. All coordinate extraction is purely geometric; no image inspection is used.

## Requirements

```bash
pip install pymupdf        # required
pip install matplotlib     # optional — generates a line plot of the extracted data
```

## Usage

```
/pdf2plot path/to/figure.pdf
```

The skill outputs a CSV alongside the PDF and prints a summary of extracted lines and their coordinate ranges.

---

## Instructions for Claude

When this skill is invoked with a PDF path as `args`, follow these steps exactly.

### Step 1 — Write the helper script

Check whether `pdf2plot_helper.py` exists in the current working directory. If not, write it now using the code block at the bottom of this file.

### Step 2 — Render the PDF and extract paths

```bash
python pdf2plot_helper.py render <pdf_path>
```

This produces one file next to the PDF:
- `<stem>_paths.json` — all vector paths with pixel coordinates, auto-classification, and extracted axis labels

**All coordinate extraction is purely geometric.** Data point positions come from the vector path coordinates. Axis label values and their calibration pixel positions are extracted from the PDF text layer — no image inspection is needed.

When no tick paths are found the script prints ready-to-use calibration anchors derived from the axis label text and positions:

```
Axis labels extracted from PDF geometry (use these as calibration anchors):
  --y-bottom @780:0
  --y-top    @30:800
  --x-left   @150:0
  --x-right  @1448:30
  --data <id1> [<id2> ...]   (data path IDs: [...])
```

### Step 3 — Identify calibration anchors and data path IDs

**If tick mark paths were found** (`tick: N` in the render output):

Use path IDs for the two outermost ticks on each axis.

| Anchor | What to find |
|---|---|
| `--x-left ID:VAL` | Path ID of a tick near the left end of the X-axis, and its label value |
| `--x-right ID:VAL` | Path ID of a tick near the right end of the X-axis, and its label value |
| `--y-bottom ID:VAL` | Path ID of a tick near the bottom of the Y-axis, and its label value |
| `--y-top ID:VAL` | Path ID of a tick near the top of the Y-axis, and its label value |

**If no tick paths were found:** use the `--y-bottom`, `--y-top`, `--x-left`, `--x-right` values printed by the render command directly. They are computed from the PDF text layer and require no adjustment.

**Data lines** are `type: "data"` paths. Identify each series by its `color` field in the paths JSON. Exclude any path spanning the full canvas (those are background rectangles).

**Log scale**: add `--x-log` / `--y-log` if tick labels are powers of 10.

### Step 4 — Report the intermediate calibration state

Before running the export, print a summary (mirroring the manual review step in the web app):

```
Calibration identified:
  X-axis: <left anchor> = <x_left_val>, <right anchor> = <x_right_val>  [linear|log]
  Y-axis: <bottom anchor> = <y_bottom_val>, <top anchor> = <y_top_val>  [linear|log]
  Data lines: paths <data_ids>
```

### Step 5 — Export to CSV

```bash
python pdf2plot_helper.py export <stem>_paths.json \
  --x-left  <anchor>  --x-right  <anchor>  \
  --y-bottom <anchor> --y-top    <anchor>  \
  --data <id1> [<id2> ...]                 \
  [--x-log] [--y-log]
```

Where each `<anchor>` is either `ID:VAL` (path ID) or `@PX:VAL` (pixel coordinate).

The script writes `<stem>.csv` with columns `series,x,y`, prints per-series x/y ranges, and (if matplotlib is installed) saves `<stem>.png` — a line plot of the extracted data.

### Step 5.5 — View the extracted data plot

Read `<stem>.png` with the Read tool. Verify:
1. The number of lines matches the figure.
2. The x and y ranges look plausible for the source figure.
3. The line shapes look sensible (smooth trajectories, no obvious jumps or inversions).

If the ranges look wrong, check the calibration anchors and re-run the export.

### Step 6 — Report the result

Tell the user:
- Path to the output CSV
- Number of lines and point counts
- The x and y ranges for each line (printed by the export script)
- Any uncertainty about tick identification

---

## Helper script

Write this to `pdf2plot_helper.py` in the current directory if it does not already exist.
    ...
    for drawing in page.get_drawings():
        ...
        for item in drawing['items']:
            ...
            if kind == 'm':
                ...
                if len(item) == 3:  # standalone segment: ('l', start, end)
                    if current is None:
                        points.append(canvas_pt(item[1], scale, page_height))
                    points.append(canvas_pt(item[2], scale, page_height))
                    current = item[2]
                else:  # continuation: ('l', endpoint)
                    p = item[1]
                    points.append(canvas_pt(p, scale, page_height))
                    current = p
            elif kind == 'c':
