...
const H = 841.89
const MARGIN = 48
const GREEN = rgb(0.043, 0.451, 0.314) // --kf-green
const INK = rgb(0, 0, 0)
...
export async function buildQuotePdf(data: QuoteDocData): Promise<Uint8Array> {
  ...
  text('Kyfaru', MARGIN, y - 2, { size: 16, bold: true, color: GREEN })
  text('TECH WITH HORNS', MARGIN, y - 16, { size: 7.5, color: MUTED })
  text('info@kyfaru.com  ·  +254 705 256 443', MARGIN, y - 30, { size: 8.5, color: MUTED })
  y -= 60

  // Title
  text('QUOTATION', MARGIN, y, { size: 22, bold: true, color: INK })
  y -= 8
  page.drawLine({ start: { x: MARGIN, y }, end: { x: W - MARGIN, y }, thickness: 1.5, color: GREEN })
  y -= 28
  ...
  const drawHeader = () => {
    page.drawRectangle({ x: MARGIN, y: y - 16, width: W - MARGIN * 2, height: 22, color: GREEN })
  ...
  y -= 18
  const policy = 'New features requested outside this scope require a separate quote and invoice.'
  const policyLines = wrapText(policy, robotoBold, 8.5, W - MARGIN * 2 - 16)
  const policyBoxH = policyLines.length * 12 + 12
  page.drawRectangle({ x: MARGIN, y: y - policyBoxH + 10, width: W - MARGIN * 2, height: policyBoxH, color: rgb(1, 0.97, 0.88), borderColor: rgb(0.96, 0.76, 0.25), borderWidth: 1 })
  let policyY = y
  for (const line of policyLines) {
    text(line, MARGIN + 8, policyY, { size: 8.5, bold: true, color: rgb(0.55, 0.4, 0.05) })
    policyY -= 12
  }
  y -= policyBoxH + 8
